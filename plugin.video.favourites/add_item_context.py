...
def add_to_directory(items):
    path, label, thumb, fanart, playable = getCurrentParams()
    if len(items) > 0:
        select = xbmcgui.Dialog().select("Wybór folderu", [x.get("label") for x in items])
        if select > -1:
            # selected_path = items[select]['path']
            selected_name = items[select]['label']
            favourites_file = 'special://profile/addon_data/plugin.video.favourites/folders/%s/favourites.xml' % selected_name
            name = label
            # url = 'ActivateWindow(videos,&quot;%s&quot;,return)' % path
            main.add_favourite(favourites_file, name, path, thumb, fanart, playable)
        else:
            xbmcgui.Dialog().notification("Info", "Nie wybrano folderu")
            sys.exit()
    else:
        xbmcgui.Dialog().notification("Błąd", "Brak folderów")
        sys.exit()
...
def getCurrentParams():
    ...
    if not thumb:
        thumb = xbmc.getInfoLabel('%sListItem.Art(poster)' % prefix)
    ...
    return path, label, thumb, fanart, playable
