...
class source:
    ...
    def tvshow(self, imdb, tmdb, tvshowtitle, localtvshowtitle, aliases, year):
        ...
        return {cleantitle.normalize(cleantitle.getsearch(tvshowtitle)),
            ...
            for link in links:
                try:
                    ...
                    headers = {
                        ...
                        "Referer": self.base_link + url, "TE": "Trailers", }
                    result = self.work(link, hostDict, headers)
                    sources.append(
                        {"source": result[0], "quality": result[1], "language": "pl", "url": result[2], "info": "",
