# -*- coding: utf-8 -*-


"""
    FanFilm Project
"""

import re
from urllib.parse import parse_qs, urljoin, urlencode

from lib.ff import client
from lib.ff import dom_parser
from lib.ff.log_utils import LOGDEBUG, LOGERROR, LOGINFO, LOGWARNING, fflog


class source:
    def __init__(self):
        self.priority = 1
        self.language = ["en"]
        self.domains = ["vidsrc.me", "v2.vidsrc.me"]
        self.base_link = "https://v2.vidsrc.me"
        self.movie_link = "/embed/%s"
        self.tv_link = "/embed/%s/%s-%s"
        self.headers = {"User-Agent": client.agent(), "Referer": self.base_link}

    def movie(self, imdb, title, localtitle, aliases, year):
        try:
            url = {"imdb": imdb, "title": title, "year": year}
            url = urlencode(url)
            return url
        except Exception:
            return

    def tvshow(self, imdb, tmdb, tvshowtitle, localtvshowtitle, aliases, year):
        try:
            url = {"imdb": imdb, "tmdb": tmdb, "tvshowtitle": tvshowtitle, "year": year}
            url = urlencode(url)
            return url
        except Exception:
            return

    def episode(self, url, imdb, tmdb, title, premiered, season, episode):
        try:
            if url is None:
                return

            url = parse_qs(url)
            url = dict([(i, url[i][0]) if url[i] else (i, "") for i in url])
            url["title"], url["premiered"], url["season"], url["episode"] = (title, premiered, season, episode,)
            url = urlencode(url)
            return url
        except Exception:
            return

    def sources(self, url, hostDict, hostprDict):
        sources = []
        try:
            if url is None:
                return sources

            hostDict = hostprDict + hostDict

            data = parse_qs(url)
            data = dict([(i, data[i][0]) if data[i] else (i, "") for i in data])

            if not data["imdb"] or data["imdb"] == "0":
                return sources

            if "tvshowtitle" in data:
                query = self.tv_link % (data["imdb"], data["season"], data["episode"])
            else:
                query = self.movie_link % data["imdb"]

            url = urljoin(self.base_link, query)
            # fflog('VIDSRC url: ' + repr(url))

            r = client.r_request(url)
            # fflog('VIDSRC r: ' + r)
            items = dom_parser.parse_dom(r, "div", req="data-hash")
            for item in items:
                url = "https://v2.vidsrc.me/src/%s" % item.attrs["data-hash"]
                # fflog('VIDSRC url: ' + repr(url))
                host = client.parseDOM(item.content, "div")[0]
                # fflog('VIDSRC host: ' + repr(host))
                host = host.lower().replace("vidsrc", "").strip()
                if host == "pro":  # other sources are javascripted
                    host = "direct"
                    sources.append({"source": host, "quality": "720p", "language": "en", "url": url, "direct": True,
                        "debridonly": False, })
            return sources
        except Exception:
            fflog("VIDSRC Exception")
            return sources

    def resolve(self, url):
        data = client.r_request(url)
        # fflog('VIDSRC data: ' + data)
        try:
            link = re.findall('"file": "(.+?)"', data)[0]
        except Exception:
            link = re.findall("'player' src='(.+?)'", data)[0]
        link = link + "|Referer=https://vidsrc.me"
        url = link if link.startswith("http") else "https:{0}".format(link)
        # fflog('VIDSRCurl: ' + repr(url))
        return url
